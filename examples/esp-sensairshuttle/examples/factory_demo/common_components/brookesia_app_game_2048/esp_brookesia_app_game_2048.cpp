...
bool Game2048::run()
{
    ...
    // Only initialize random seed if starting a new game
    if (!_is_game_initialized) {
        srand(time(nullptr));
    }
    ...
    lv_obj_align(cur, LV_ALIGN_TOP_LEFT, 50, 10);
    ...
    lv_obj_align(score_title, LV_ALIGN_TOP_MID, -5, 5);
    ...
    lv_obj_align(best, LV_ALIGN_TOP_RIGHT, -50, 10);
    ...
    lv_obj_set_size(grid, _width, _width - 10);
    ...
    lv_obj_align(_foreground_grid, LV_ALIGN_CENTER, 0, 25);
    ...
    // Only start a new game if this is the first time running
    if (!_is_game_initialized) {
        newGame();
        _is_game_initialized = true;
    } else {
...
bool Game2048::close()
{
    ...
    // Set both flags to prevent any gesture processing during close
    _is_closing = true;
    _is_paused = true;
    ...
    // Reset game initialization flag when app is closed
    // This ensures a fresh game when app is reopened
    _is_game_initialized = false;
...
void Game2048::restoreGameState()
{
    // Restore score displays
    if (_cur_score_label) {
        updateCurrentScore(_current_score);
    }
    if (_best_score_label) {
        updateBestScore(_best_score);
    }

    // Save the weight data before cleaning UI
    cell_weight_t saved_weights[4][4];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            saved_weights[i][j] = _cells_weight[i][j];
        }
    }

    // Rebuild foreground cells based on saved weights
    // Only clean UI objects, not the weight data
    if (_foreground_grid != nullptr) {
        lv_obj_t *child = lv_obj_get_child(_foreground_grid, 0);
        while (child) {
            lv_obj_del(child);
            child = lv_obj_get_child(_foreground_grid, 0);
    ...
    // Clear foreground cells pointers
    for (int i = 0; i < 16; i++) {
        _foreground_cells[i / 4][i % 4] = NULL;
    }

    // Restore weight data and rebuild cells
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            _cells_weight[i][j] = saved_weights[i][j];
            if (_cells_weight[i][j].weight > 0) {
                /* Add a new object of cell */
                lv_obj_t *cell = lv_obj_create(_foreground_grid);
                _foreground_cells[i][j] = cell;
                // Size
                lv_obj_set_size(cell, CELL_SIZE, CELL_SIZE);
                // Position
                lv_obj_set_pos(
                    cell,
                    lv_obj_get_x(_background_cells[i][j]),
                    lv_obj_get_y(_background_cells[i][j])
                );
                // Shape
                ...
                lv_label_set_text_fmt(label, "%d", 1 << _cells_weight[i][j].weight);
                lv_obj_center(label);
            }
        }
    }
...
void Game2048::motion_event_cb(lv_event_t *e)
{
    ...
    // If the app is closing or paused, return immediately to prevent gesture conflicts
    // This prevents game gestures from being processed when system gestures (like returning home) are triggered
    if (app->_is_closing || app->_is_paused) {
        return;
    }

    if (!app->_anim_running_flag) {
        switch (type->direction) {
        case systems::phone::Gesture::DIR_UP:
            // printf(NULL, "up");
            score = app->moveUp();
            break;
        case systems::phone::Gesture::DIR_DOWN:
            // printf(NULL, "down");
            score = app->moveDown();
            break;
        case systems::phone::Gesture::DIR_LEFT:
            // printf(NULL, "left");
            score = app->moveLeft();
            break;
        case systems::phone::Gesture::DIR_RIGHT:
            // printf(NULL, "right");
            score = app->moveRight();
            break;
        default:
...
void Game2048::anim_finish_cb(lv_anim_t *a)
{
    ...
    if (app->_anim_count > 0) {
        app->_anim_count--;
    }

    // Only execute final operations when all animations are finished
    if (app->_anim_count == 0) {
...
bool Game2048::pause()
{
    ...
    // Set paused flag immediately to prevent gesture events from being processed
    // This is critical to prevent game gestures from conflicting with system gestures
    // (e.g., when user swipes up to return home, the game should not process the up gesture)
    _is_paused = true;

    // Stop all animations to prevent callbacks from being triggered after resume
    if (_foreground_grid != nullptr) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                if (_foreground_cells[i][j] != nullptr) {
                    // Delete animations on this cell (both X and Y animations)
                    lv_anim_del(_foreground_cells[i][j], (lv_anim_exec_xcb_t)lv_obj_set_x);
                    lv_anim_del(_foreground_cells[i][j], (lv_anim_exec_xcb_t)lv_obj_set_y);
                }
            }
        }
    }

    // Reset animation state to prevent stale callbacks
    _anim_count = 0;
    _anim_running_flag = false;
    _generate_cell_flag = false;

    return true;
}

bool Game2048::resume()
{
    ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();

    // Clear paused flag to allow gesture events to be processed again
    _is_paused = false;

    // Reset animation state in case of any stale state
    _anim_count = 0;
    _anim_running_flag = false;
    _generate_cell_flag = false;

    // Restore game state if game was initialized
    // Note: UI might have been recreated by loadRecentScreen(), so we need to restore
    if (_is_game_initialized) {
        // If UI objects are still valid, restore state
        if (_foreground_grid != nullptr && _background_cells[0][0] != nullptr) {
            restoreGameState();
        }
        // If UI was destroyed, it will be recreated in run() and restoreGameState() will be called there
    }
