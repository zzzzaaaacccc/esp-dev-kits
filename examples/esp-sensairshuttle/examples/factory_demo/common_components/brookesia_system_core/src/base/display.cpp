...
#include "brookesia/system_core/gui/lvgl/container.hpp"
#include "brookesia/system_core/gui/lvgl/helper.hpp"
#include "brookesia/system_core/gui/lvgl/screen.hpp"
#include "brookesia/system_core/base/display.hpp"
...
bool Display::calibrateCoreData(Data &data)
{
    ...
    for (int i = StyleFont::FONT_SIZE_MIN; i <= StyleFont::FONT_SIZE_MAX; i += 2) {
        if (_update_size_font_map.find(i) == _update_size_font_map.end()) {
            ...
            if (!core_utils_get_internal_font_by_size(i, &font_resource)) {
                continue;
            }
