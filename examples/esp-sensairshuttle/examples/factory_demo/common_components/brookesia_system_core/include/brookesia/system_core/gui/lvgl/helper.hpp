 ...
 */
inline bool core_utils_get_internal_font_by_size(uint8_t size_px, const lv_font_t **font)
{
    return esp_brookesia::gui::getLvInternalFontBySize(size_px, font);
}
inline lv_color_t core_utils_get_random_color(void)
{
    return esp_brookesia::gui::getLvRandomColor();
}
inline bool core_utils_check_obj_out_of_parent(lv_obj_t *obj)
{
    return esp_brookesia::gui::checkLvObjOutOfParent(obj);
}
inline bool core_utils_check_event_code_valid(lv_event_code_t code)
{
    return esp_brookesia::gui::checkLvEventCodeValid(code);
}
inline lv_indev_t *core_utils_get_input_dev(const lv_display_t *display, lv_indev_type_t type)
{
    return esp_brookesia::gui::getLvInputDev(display, type);
}
inline lv_anim_path_cb_t core_utils_get_anim_path_cb(esp_brookesia::gui::StyleAnimation::AnimationPathType type)
{
