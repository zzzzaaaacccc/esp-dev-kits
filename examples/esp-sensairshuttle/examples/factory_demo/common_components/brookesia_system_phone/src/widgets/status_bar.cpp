...
{
    ESP_UTILS_LOGD("Set Visual Mode(%d)", static_cast<int>(mode));
...
bool StatusBar::updateBatteryByNewData(void)
{
    ...
    if (_data.flags.enable_battery_label) {
        ...
        if (core_utils_check_obj_out_of_parent(_battery_label.get())) {
...
bool StatusBar::updateClockByNewData(void)
{
    ...
    if (core_utils_check_obj_out_of_parent(_clock_obj.get())) {
